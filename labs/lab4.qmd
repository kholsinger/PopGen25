...
freq_GT = function(site,ploidy=2){
  ...
  p = sum(clean_site)/(???) #How will ploidy go in here?
  return(p)
}
```
...
Now, write a function that will take a GT matrix and thin it to whatever proportion the user desires.
...
-r2 gz \ #Output the r2 statistic, and gzip it so it is not as giant.
