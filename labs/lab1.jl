...
using Plots, PlutoUI, Distributions, Measures, LaTeXStrings, PlutoExtras, PlutoTeachingTools, CSV, DataFrames
...
In Pluto, everything is structured as a bunch of `cells`. Each `cell` can contain code. Even text (like this) is really just markdown under the hood. Notice a few bits of UI before we start.

1) Look to the top right. There might be a button there that says "Run this notebook". Click that! A bunch of things will start happening as the notebook compiles and runs in the background.
...
4) There's a plus icon above and below the text block, just to the left. Clicking these will create a new cell above/below the current one.

Now that you are hopefully slightly oriented, let's do some coding!
"""
...
To make this scalable, we'll write two functions. The first is the backbone of the simulations - it's a loop that takes a bunch of initial parameters and returns a time series of allele frequencies.
...
But, what is the `randomMating()` function? Well, we define it below. 
"""
...
md"""
And here we run the actual random sampling function. Recall from lecture that it's really just a `Binomial` sample with probability $p$ and $N$ new individuals each generation, but we need the proportion of successes, so we divide the result by $N$.
...
md"""
An odd, but in some ways nice, thing about Pluto is that the bindings of the parameters can change in later cells (the whole thing is evaluated for inter-dependencies, and cells are re-run in the necessary order). So, even though we run the simulations in cells above, if you change the number of generations we simulate for (_t_) or the population size (_N_) below, you'll update the figure above as well!
...
md"""
## Going beyond standard Wright-Fisher

Today we talked about how the standard assumptions of the WF model can often be broken. Indeed, we saw how some alleles increase in frequency much more rapidly than expected under WF.

There are two potential reasons in these populations - they don't experience migration, and mating is as random as they can manage, so the allele frequency wonkiness is either due to mutation, or selection.

### Mutation


One of the most important ways WF is wrong is in not accounting for `mutation`. This is a fairly simple thing to include in our broad model, since we can make mutation quite general.
...
if !@isdefined(mutation)
	...
	let
		result = mutation(0.1,0.1)
		if ismissing(result)
			still_missing()
		elseif isnothing(result)
			keep_working(md"Did you forget to write `return`?")
		elseif result == 0.1
			keep_working(md"Edit the code so that next = $p(1-\mu) + (1-p)\mu$")
		elseif !isapprox(result,0.18)
			keep_working(md"Not quite right yet. If you are having trouble entering μ into the code, type \mu and hit Tab.")
		else
			correct()
		end
	end
...
_μ_ = $(@bind μ NumberField(0:0.0001:0.5;default=0.01))
...
Selection can be implemented in a variety of ways. For now, we'll implement the most general of these. Let's suppose the number of gametes is still infinite, but the proportion of gametes that each individual contributes now depends on its genotype. The fitness of `aa` will be considered the reference fitness, so their fitness ($W_{aa}$) will be set to 1. We can then specify the relative fitness of `Aa` and `AA` individuals. From theory you'll learn in a few weeks, the frequency of the allele after selection is:

"""

# ╔═╡ 5908f3ea-77ad-44f4-a985-22fb752b2b93
texeq"p` =  \frac{p_{AA}w_{AA}+1/2p_{Aa}w_{Aa}}{1+p_{AA}(w_{AA}-1)+p_{Aa}(w_{Aa}-1)} \label{sel}"

# ╔═╡ aa82cb4b-e23a-4748-9d1f-5153535e0978
md"""
In the above:

$w_{AA}, w_{Aa}$ are the relative fitnesses of `AA` and `Aa` individuals, respectively,

$p_{AA}, p_{Aa}$ are their frequencies. 

If the population is in Hardy-Weinberg before gametes are produced (it should be), then you should already know how to go from $p$ to $p_{AA}$.
...
if !@isdefined(sel)
	...
	let
		result = sel(1.0,2.0,0.1,0.1)
		if ismissing(result)
			still_missing()
		elseif isnothing(result)
			keep_working(md"Did you forget to write `return`?")
		elseif result != 0.2/1.1
			keep_working(md"Not quite right yet. Equation 1 is quite long - if you find it hard to write as a single line, consider writing two lines (denominator and numerator), and return not next but denominator/numerator")
		else
			correct()
		end
	end
...
let
	seed=0
	result = WrightFisher(100000,0.01,2,0.0,10)[2]
	if ismissing(result)
		still_missing()
	elseif isnothing(result)
		keep_working(md"It seems that your code is still broken.")
	elseif result <= 0.02
		tip(md"The above simulation plot probably looks the same. Don't forget that we need to modify the actual WrightFisher simulation code to include selection.")
	elseif result <0.2
		tip(md"Seems like you've modified the simulations in an unexpected way. Ask for help!")
	else
		correct(md"Seems like it's working ok, but this simulation is stochastic.")
	end
end
