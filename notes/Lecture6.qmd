...
theme(:juno)
default(xlabelfontsize=16,ylabelfontsize=16,margins=4mm,markerstrokewidth=0,markersize=2,dpi=400)
...
```{julia}
ps2 = Ns/sum(Ns)
bar(["aa","aA","AA"],ps,leg=false,xlabel="Genotype",ylabel="Frequency of offspring",size=(1000,600))
```

## And convert to HW expectation in next generation:

```{julia}
p2 = 0.5*ps2[2]+ps2[3]
Ns2 = [(1-p2)^2,2*p2*(1-p2),p2^2]
bar(["aa","aA","AA"],Ns2,leg=false,xlabel="Genotype",ylabel="Frequency in next generation",size=(1000,600))
```
...
Almost universally, the change in allele frequency will consist of two major terms: Strenght of selection and the amount of genetic variability

. . .

This is why we have focused so much on what creates/destroys genetic variation - without it there is no selection and adaptation.
...
## What does selection *do*? {.smaller}
...
Barton showed that [@Barton2000]

$$
\frac{\pi}{\pi_0} = 1-(4Ns)^{2r/s}
$$

e.g. the reduction in genetic diversity is predictable.

. . .

## Theoretical reduction in diversity

```{julia}
function pi_red(x,N,s)
    return(1-(4*N*s)^(-2*x/s))
end
dists = vcat(collect(range(0.01,0,length=50)),collect(range(0,0.01,length=50)))
pis1 = [pi_red(x,10000,0.01) for x in dists]
dists2 = vcat(collect(range(-0.01,0,length=50)),collect(range(0,0.01,length=50)))
plot(dists2,pis1,size(1000,600),xlabel="Genetic Distance",ylabel=L"\frac{\pi}{\pi_0}",label=L"4Ns=100")
pis2 = [pi_red(x,10000,0.1) for x in dists]
plot!(dists2,pis2,label=L"4Ns=1000")
```
...
That requires you to build a strong sense of intuition for what these different statistics mean. This week for reading: we'll look at adaptation in A. Aedes.
