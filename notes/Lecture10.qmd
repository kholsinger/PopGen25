...
## Do we care about _all_ recombination events?

Recombination events in individuals among segments not inherited to our samples don't actually matter.

. . . 

Even in the wors case of very high recombination rates, we eventually create lots of infinitesimal segments that are unlikely to recombine, and so are forced to coalesce.
...
So, we know that, mathematically, you can eventually get the full geneological history of a sample.

. . . 

That history contains the _full_ information about what has happened in the ancestors of your samples.
...
When adding recombination, we now not only don't have an exact solution for expected coalescene time (and therefore total expected mutations), but different segments may have different *marginal* histories.
...
Some samples will simply be more/less related to each other in some _parts_ of the chromosome due to the ARG.

. . . 

Even if we have perfect mutation/recombination rate knowledge, there are many evoutionary histories that are consistent with observed mutations.

. . . 

The parameter space to explore is gigantic even for a few samples, since recombination means the number of lineages to keep track of is not $1:n$, but $1:\infty$, even if the tail of the distribution is _very_ thin.

## The full ARG - impossible to obtain
...
Here, we _can_ use simple coalescent approximation approaches - no recombination along entire segment.
...
The SMC approach is not _quite_ the full ARG, nor just the coalescent with recombination.

. . . 

In particular, SMC works by ignoring certain histories (for instance, segments which are "floating" and coalesce into the same lineage), so it is not the "full" ARG.

. . . 

But it turns out the "full" ARG contains a lot of redundant information, so SMC captures a lot of the data needed.
...
In short: if you can model it using the coalescent (and that's pretty much everything in evolutionary biology), you can estimate it using some SMC appraoch.

## Big caveats {.smaller}

The samples that exist now show you the history of the samples that survived.
...
As with any model fitting, it's tempting to just fit _everything_. But, to fit everything you need more samples, and more samples means more genealogies. That means you are always running the dual risk of over-fitting by having complex models for few samples, and under-fitting by oversimplifying dynamics if you have many samples. There is no practical rule of thumb here - try multiple models and see how robust your results are.

## Practical approaches for ARG inference.
