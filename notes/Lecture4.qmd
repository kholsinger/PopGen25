...
## Some true facts about branching processes {.smaller}
...
Anything that creates *joint* identity-by-descent (some examples given).

. . .

Population structure/migration [@Price2008].
...
Direct selection (but only weakly) [@Liu2014].

. . .

Epistatic selection (selection on combination of alleles) [@Nagylaki1993].
...
Some wailing and gnashing of teeth later, if we assume $r \ll 0.5$, we can get:
...
## Things you will read/think that are subtly wrong: {.smaller}
...
-   ROH - Runs of Homozygosity. We mentioned these previously. They can be indicators of inbreeding, since:
    ...
    -   The length of such regions can tell you how far from the inbreeding event this chunk descended from (since only recombination will break up the ROH). \## cntd.
...
You might try to first look at divergence across the genome to detect *what* is causing this interesting trait to appear.
...
So far we have been agnostic about *what* our genes do, because we haven't needed to worry about it.
...
In other words - the trait is *correlated* to the population's ancestry as a whole - there's LD here.
