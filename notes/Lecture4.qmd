...
## Asexual recombination cntd {.smaller}
...
anim = @animate for i ∈ 2:101
    plot(1:i,transpose(ds[:,1:i]),leg=false,xlabel="Generation",ylabel="D",xlims=(0,100),palette=cols,ylims=(0,0.25),margins=4mm)
end
gif(anim,fps=10)
```
...
More formally, let $Q_n$ be the probability that two alleles are identical by descent after $n$ generations
...
contourf(10 .^N,10 .^r,[rho_bar(x,y) for x in 10.0 .^N, y in 10.0 .^r],xaxis=:log10,yaxis=:log10,xlabel="N",ylabel="r",margins=4mm)
```

## Things you will hear/read/think that are subtly wrong:

-   These loci are only in LD because of proximity

    -   How did that LD build up to begin with?
...
-   I can just LD prune my data and get the same results.
    ...
    -   We'll chat more about this one.

## LD and pop gen analyses
...
## Other ways of dealing with LD: LD-decay

As you saw, LD decays fairly quickly with increasing recombination distance.

. . .

So, we can instead examine individual windows, spaced apart by roughly how far $r^2$ drops to 1/2.
...
p1 = scatter(dist,muc_data.R2,leg=false,xlabel="Distance between SNPs",ylabel=L"r^2",margins=5mm,size=(800,600);markersize=1,markerstrokewidth=0)
```
...
p2 = scatter(subdata.BP_A,
    ...
    ylabel="SNP 2 position",
    clabel=L"r^2",margins=4mm;markerstrokewidth=0,markersize=1,clims=(0.0,1.0))
```
